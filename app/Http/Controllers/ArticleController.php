...
public function articleshow($slug)
{
	...
	return view('articleshow')->with(compact('show','allposts','randart','randoc','randpen','ad'));
}
...
public function articleblog()
{
	...
	return view('articleblog')->with(compact('allposts','randart','randoc','randpen','ad'));
}
...
$randart = a::join('users','article.artwriter','=','users.id')->select('article.*','users.name')->inRandomOrder()->paginate(5);
        $randoc = u::where('role',2)->inRandomOrder()->paginate(2);
        $randpen = p::inRandomOrder()->paginate(5);
	return view('categoryblogs')->with(compact('allcate','randart','randoc','randpen','ad'));
}
...
else
{
 ...
 $msg = notify()->flash('Category berhasil diubah', 'success');
    ...
    public function categoryshow($slug)
    {
        ...
        $randoc = u::where('role',2)->inRandomOrder()->paginate(2);
        $randpen = p::inRandomOrder()->paginate(5);
        return view('categoryshows')->with(compact('a','randart','randoc','randpen','findslug','ad'));
    }
}
